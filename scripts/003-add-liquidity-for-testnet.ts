...
const func: DeployFunction = async function (hre: HardhatRuntimeEnvironment) {
    ...
    if (network.name == "amoy") {
        ...
        const uniswapFactory = (await config.get(
            "uniswap_v2_factory"
        )) as string;
        const uniswapRouter = (await config.get(
            "uniswap_v2_router_02"
        )) as string;
        const teleBTC = await deployments.get("TeleBTCProxy");

        const teleBTCFactory = await ethers.getContractFactory("TeleBTCLogic");
        const teleBTCInstance = await teleBTCFactory.attach(teleBTC.address);
        ...
        if (theLiquidityPair2 == "0x0000000000000000000000000000000000000000") {
            ...
            if (!isMinterTeleBTCTx) {
                const addMinterTeleBTCTx = await teleBTCInstance.addMinter(
                    deployer
                );
                await addMinterTeleBTCTx.wait(1);
            }

            const mintTeleBTCTx = await teleBTCInstance.mint(
                deployer,
                one8Dec.div(2)
            );
            ...
            console.log(
                "Charged WMATIC-TELEBTC pool: ",
                addLiquidityPairTx.hash
            );
        }
    }
};
