...
contract CCBurnRouter is ICCBurnRouter, Ownable, ReentrancyGuard {
    ...
    // Private variables
    mapping(bytes32 => bool) private isUsedAsBurnProof; // Mapping that shows a txId has been submitted to pay a burn request 
    ...
    /// @param _userLockingScript           User's locking script
    ...
    function burnProof(
        ...
        /* 
            ...
            _endIndex should be less than total assigned requests to the locker
        */
        require(
        ...
        */
        ...
        for (uint i = _startIndex; i <= _endIndex; i++) {
            ...
            if (
                ...
                burnRequests[_lockerTargetAddress][i].deadline >= _lastSubmittedHeight
            ) {
    ...
    function _updateIsUsedAsBurnProof(
        ...
        uint parsedAmount = TxHelper.parseOutputValue(_vout, _lockerLockingScript);
        ...
        } else if (parsedAmount == 0 && _paidOutputCounter == numberOfOutputs) {
