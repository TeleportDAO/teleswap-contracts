...
//     ) external view returns (uint64 bitcoinAmount, bytes memory arbitraryData) {
...
//         bytes memory _vout,
//         bytes memory _lockingScript
//     ) external view returns (uint64 bitcoinAmount) {
//         bytes29 voutView = _vout.ref(0).tryAsVout();
//         require(!voutView.isNull(), "TxHelper: vout is null");
...
//         TxHelper.ScriptTypes _scriptType
//     ) external view returns (uint64 bitcoinAmount) {
...
//         bytes memory _vout, 
//         uint _index
//     ) external view returns (bytes memory _lockingScript) {
...
//         bytes memory _vin, 
//         uint _index
//     ) external pure returns (bytes32 _txId, uint _outputIndex) {
...
//     function parseTotalValue(bytes memory vout) external pure returns (uint64) {
...
//     function parseChainId(bytes memory arbitraryData) external pure returns (uint8 parsedValue) {
...
//     function parseAppId(bytes memory arbitraryData) external pure returns (uint16 parsedValue) {
...
//     function parseRecipientAddress(bytes memory arbitraryData) external pure returns (address parsedValue) {
...
//     function parsePercentageFee(bytes memory arbitraryData) external pure returns (uint16 parsedValue) {
...
//     function parseSpeed(bytes memory arbitraryData) external pure returns (uint8 parsedValue) {
...
//     function parseExchangeToken(bytes memory arbitraryData) external pure returns (address parsedValue){
...
//     function parseExchangeOutputAmount(bytes memory arbitraryData) external pure returns (uint224 parsedValue){
...
//     function parseDeadline(bytes memory arbitraryData) external pure returns (uint32 parsedValue){
...
//     function parseIsFixedToken(bytes memory arbitraryData) external pure returns (uint8 parsedValue){
...
//     // function parseIsExchange (bytes memory arbitraryData) external returns (bool parsedValue) {
...
//     // function parseSpeed(bytes memory arbitraryData) external returns (uint8 parsedValue){
...
//     function revertBytes32(bytes32 input) external pure returns (bytes32) {
//         bytes memory temp;
//         bytes32 result;
//         for (uint i = 0; i < 32; i++) {
...
//     function parseInput(bytes memory vin, uint index) external pure returns (bytes29 input) {
//         bytes29 vinView = vin.ref(0).tryAsVin();
//         // Extract the desired input
//         input = ViewBTC.indexVin(vinView, index);
//     }

//     function parseInputScriptSig(bytes memory vin, uint index) external view returns (bytes memory scriptSig) {
//         // Extract the desired input
...
//     function numberOfOutputs(bytes memory vout) external pure returns (uint _numberOfOutputs) {
//         bytes29 voutView = vout.ref(0).tryAsVout();
