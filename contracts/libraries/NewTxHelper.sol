...
library NewTxHelper {
    ...
    function parseExchangeOutputAmount(bytes memory arbitraryData) internal returns (uint128 parsedValue){
        bytes memory slicedBytes = sliceBytes(arbitraryData, 46, 61);
        assembly {
    ...
    function parseDeadline(bytes memory arbitraryData) internal returns (uint128 parsedValue){
        bytes memory slicedBytes = sliceBytes(arbitraryData, 62, 77);
        assembly {
            parsedValue := mload(add(slicedBytes, 16))
        }
