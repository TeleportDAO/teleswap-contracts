...
describe('Relay', async () => {
    ...
    //     const bitcoinRelayFactory2 = new BitcoinRelay__factory(
    //         _signer || deployer
    //     );
    ...
    //     const bitcoinRelay = await bitcoinRelayFactory2.deploy(
    //         _genesisHeader,
    //         _heightBigNumber,
    //         _periodStart,
    //         ZERO_ADDRESS,
    ...
    describe('Submitting block headers with retarget', async () => {
        ...
        before(async () => {
            ...
            bitcoinRelay = await bitcoinRelayFactory.deploy(
                _genesisHeader,
                _heightBigNumber,
                _periodStart,
                ZERO_ADDRESS,
        ...
        it('submit old block headers', async () => {
            // this.timeout(0);
    ...
    describe('Check tx inclusion', async () => {
        ...
        it('check the inclusion of a transaction having txid', async() => {
            ...
            let theCheckingResult = await bitcoinRelay.checkTxProof(
                _txid,
                _blockNumber,
                _intermediateNodes,
                _index,
            ...
            // FIXME: how to get the return when the message has value
            // expect(
            //     theCheckingResult
            // ).to.equal(true);
        });
