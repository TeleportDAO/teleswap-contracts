...
describe("Bitcoin Relay", async () => {
    ...
    describe('Submitting block headers', async () => {
        it('submit too old block headers to form a fork (unsuccessful)', async function () {
            /* eslint-disable-next-line camelcase */
            const { mainChain, forkedChain } = FORKEDCHAIN;
            for (let i = 1; i < 8; i++) {
                bitcoinRelay.addHeaders(
            ...
            // genesis height is 6605
            // in forkedChain block 6610 is the first block different from its same height block in mainChain
            for (let i = 5; i < 8; i++) {
                bitcoinRelay.addHeaders(
            ...
            // No new block header gets accepted on top of finalized block headers
            let blockHeaderNew = await bitcoinRESTAPI.getHexBlockHeader(100*2016);
            let blockHeaderOld = '';
            ...
            ).to.not.emit(bitcoinRelay, "BlockAdded")
    ...
    //         await expect(
    //             instance.addHeaders(genesis.hex, header)
